...
public class SpecificationController {
    ...
    @ApiImplicitParam(paramType = "path", name = "id", value = "主键ID", required = true, dataType = "Long")
    @PutMapping(value="/{id}")
    public Result update(@RequestBody @ApiParam(name = "Specification对象",value = "传入JSON数据",required = false) SpecificationItemOption specificationItemOption,@PathVariable Long id){
        //设置主键值
        specificationItemOption.getSpecification().setId(id);
        //调用SpecificationService实现修改Specification
        specificationService.update(specificationItemOption);
    ...
    @PostMapping
    public Result add(@RequestBody  @ApiParam(name = "Specification对象",value = "传入JSON数据",required = true) SpecificationItemOption specificationItemOption){
        //调用SpecificationService实现添加Specification
        specificationService.add(specificationItemOption);
    ...
    @ApiImplicitParam(paramType = "path", name = "id", value = "主键ID", required = true, dataType = "Long")
    @GetMapping("/{id}")
    public Result<SpecificationItemOption> findById(@PathVariable Long id){
        //调用SpecificationService实现根据主键查询Specification
        SpecificationItemOption specificationItemOption = specificationService.findById(id);
        return new Result<SpecificationItemOption>(true,StatusCode.OK,"查询成功",specificationItemOption);
    }
     ...
     * @return
     */
    @ApiOperation(value = "查询Specification列表",notes = "查询Specification列表详情",tags = {"SpecificationController"})
    @GetMapping("/findOptions")
    public Result<List<Map>> findOptions(){
        //调用SpecificationService实现查询所有Specification
        return new Result<List<Map>>(true, StatusCode.OK,"查询成功",specificationService.findOptions()) ;
    }
}
